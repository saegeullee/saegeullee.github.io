...
다음 명령어를 통해 컬렉션에서 distance 프로퍼티가 900 이상인 도큐먼트만을 찾을 수 있다. `$gt`의 gt는 greater than의 이니셜이다.
...
다음 명령어 `findOne`은 특정 기준을 만족하는 단 하나의 도큐먼트만을 찾는다.
...
몽고DB에서 컬렉션의 도큐먼트를 찾을때는 find 함수를 사용한다. find 함수는 도큐먼트를 곧바로 리턴하지 않고 `커서(Cursor) 객체`를 리턴한다. 만약 컬렉션에 20개 이상의 도큐먼트가 있을때 find 함수로 모든 도큐먼트를 찾으면 20개의 도큐먼트가 나오고 맨 아래에 다음과 같이 나타난다.
